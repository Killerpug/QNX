...
int main()
{
	...
	/*
	 * adjust the system tick to be 500 us
	 */
	new.nsec = 500 * 1000;
	new.fract = 0;
	if (-1 == ClockPeriod(CLOCK_REALTIME, &new, &old, 0))
	{
		perror("ClockPeriod");
		exit(EXIT_FAILURE);
	}
	...
	/*
	 * allow the hardware timer chip to settle before querying
	 * the system clock
	*/
	...
	/*
	 * restore the original clock period
	 */
	if (-1 == ClockPeriod(CLOCK_REALTIME, &old, NULL, 0))
	{
		perror("ClockPeriod");
		exit(EXIT_FAILURE);
	}
